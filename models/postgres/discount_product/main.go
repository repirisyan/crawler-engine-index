...
// StoreDiscountSummaries replaces all rows for (year, month) with the given set,
// so re-running the cleaning pipeline in the same month does not double-count.
func StoreDiscountSummaries(discounts []DiscountSummaries, year int, month int) error {
	...
	if _, err := tx.Exec(Ctx,
		`DELETE FROM product_discount_summaries WHERE year = $1 AND month = $2`,
		year, month,
	); err != nil {
		log.Printf("Failed to clear discount summaries for %d-%02d: %v", year, month, err)
		return err
	}
