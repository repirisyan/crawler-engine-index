...
func GetAllData(search string) ([]Supervision, error) {
	query := "SELECT title, link, image, price, sold, seller, location, comodity_id, marketplace_id from temp_items WHERE title LIKE ?"
	likeSearch := "%" + search + "%"
	...
	for rows.Next() {
		var product Supervision
		err := rows.Scan(&product.Title, &product.Link, &product.Image, &product.Price, &product.Sold, &product.Seller, &product.Location, &product.Comodity_id, &product.Marketplace_id)
		if err != nil {
			return nil, err
		}
		products = append(products, product)
...
func StoreProduct(product Product) error {
	query := "INSERT INTO temp_items (title, link, image, price, rating, sold, seller, location, comodity_id, keyword_id, marketplace_id, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
	_, err := mysql.DB.Exec(query, product.Title, product.Link, product.Image, product.Price, product.Rating, product.Sold, product.Seller, product.Location, product.Comodity_id, product.Keyword_id, product.Marketplace_id, product.User_id)
	if err != nil {
...
func UpdateFlag(id uint64) error {
	query := "UPDATE temp_items SET FLAG = 1 WHERE id = ? "
