...
func GetBrandLeaderboard() ([]BrandLeaderboard, error) {
	...
	for cursor.Next(ctx) {
		...
		// Safely extract the values with logging on failure
		var brand, marketplace string
		var total int32

		// Extract brand
		if val, ok := raw["_id"].(bson.M)["brand"]; ok {
			if b, ok := val.(string); ok {
				brand = b
			} else {
				fmt.Printf("Warning: expected string for brand, got %T\n", val)
				continue // Skip this entry
			}
		} else {
			fmt.Println("Warning: brand not found")
			continue // Skip this entry
		}
